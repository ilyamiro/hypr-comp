...
Item {
    ...
    Shortcut {
        ...
        onActivated: {
            ...
            audioManager.playSfx("network/switch.wav");
            let modes = [];
    ...
    readonly property string scriptsDir: paths.qsDir + "/network"
    ...
    Process {
        ...
        onExited: {
            ...
            if (code !== 0) {
                ...
                audioManager.playSfx("network/error.wav"); 
    ...
    function processEthJson(textData, isCache = false) {
        ...
        try {
            ...
            if (JSON.stringify(window.ethConnected) !== JSON.stringify(newConnected)) {
                if (!window.isEthConn && newConnected && window.activeMode === "eth") audioManager.playSfx("network/connect.wav");
    ...
    function processWifiJson(textData, isCache = false) {
        ...
        try {
            ...
            if (window.activeMode === "wifi") {
                ...
                if (newlyConnected) {
                    audioManager.playSfx("network/connect.wav");
                    window.busyTasks = Object.assign({}, bt);
    ...
    property real baseRadarRotation: 0
    NumberAnimation on baseRadarRotation {
        from: 0; to: 360; duration: 24000; loops: Animation.Infinite; running: window.currentPower && !window.currentConn
    }

    property real microOrbitDrift: 0
    NumberAnimation on microOrbitDrift {
        from: 0; to: Math.PI * 2; duration: 60000; loops: Animation.Infinite; running: window.currentPower
    }
    ...
    component SatelliteLoader : Item {
        id: loaderRoot
        width: window.s(24); height: window.s(24)
        property color accentColor: window.activeColor
        
        property real rotAngle: 0
        NumberAnimation on rotAngle { from: 0; to: 360; duration: 1400; loops: Animation.Infinite; running: loaderRoot.visible }
        
        Rectangle {
            anchors.centerIn: parent
            width: window.s(14); height: width; radius: width/2
            color: "transparent"; border.color: Qt.alpha(loaderRoot.accentColor, 0.2); border.width: 1
        }
        Rectangle {
            x: window.s(12) + Math.cos(loaderRoot.rotAngle * Math.PI / 180) * window.s(7) - width/2
            y: window.s(12) + Math.sin(loaderRoot.rotAngle * Math.PI / 180) * window.s(7) - height/2
            width: window.s(4); height: width; radius: width/2
            color: loaderRoot.accentColor
            layer.enabled: true
            layer.effect: MultiEffect { blurEnabled: true; blurMax: 4; blur: 0.5 }
        }
    }
        ...
        Rectangle {
            ...
            Item {
                ...
                Repeater {
                    ...
                    Rectangle {
                        anchors.centerIn: parent
                        width: window.s(260) + (index * window.s(170))
                        height: width
                        radius: width / 2
                        color: "transparent"
                        ...
                        opacity: Object.keys(window.disconnectingDevices).length > 0 ? 0.2 : (window.currentConn ? 0.08 - (index * 0.02) : 0.04)
                ...
                Rectangle {
                    ...
                    rotation: window.baseRadarRotation
                    gradient: Gradient {
                        orientation: Gradient.Angular
                        GradientStop { position: 0.0; color: Qt.alpha(window.activeColor, 0.12) }
            ...
            Item {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        scale: centralCoreContainerItem.bumpScale * (0.8 + 0.2 * activeTransition)
                        visible: opacity > 0.01
                        ...
                        Item {
                            ...
                            Rectangle {
                                ...
                                property bool isDangerState: coreMa.containsMouse || centralCore.disconnectFill > 0 || isMyDisconnecting
                                ...
                                Rectangle {
                                    ...
                                    color: window.maroon
                                    opacity: (centralCore.isDangerState && window.currentConn && !showPassword) ? 0.15 : 0.0
                                ...
                                ColumnLayout {
                                    ...
                                    Text { Layout.alignment: Qt.AlignHCenter; font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(14); color: window.overlay0; text: window.currentPowerPending ? "Linking Core..." : "Disconnected" }
                                }
                                ...
                                Item {
                                    ...
                                    ColumnLayout {
                                        ...
                                        SatelliteLoader { Layout.alignment: Qt.AlignHCenter; visible: isMyDisconnecting; accentColor: window.overlay1 }
                                        Text {
                                            Layout.alignment: Qt.AlignHCenter
                                        ...
                                        Text {
                                            ...
                                            text: isMyDisconnecting ? "Severing..." : (centralCore.disconnectFill > 0.01 ? "Hold..." : "Connected")
                                        }
                                    }
                                    ...
                                    Item {
                                        ...
                                        ColumnLayout {
                                            ...
                                            SatelliteLoader { Layout.alignment: Qt.AlignHCenter; visible: isMyDisconnecting; accentColor: window.text }
                                            Text {
                                                Layout.alignment: Qt.AlignHCenter
                                                Layout.maximumWidth: window.s(150) - (window.s(50) * coreContainer.multiShift)
                                                horizontalAlignment: Text.AlignHCenter
                                ...
                                MouseArea {
                                    id: coreMa
                                    anchors.fill: parent
                                    hoverEnabled: window.currentConn && !isMyDisconnecting && !showPassword
                                ...
                                NumberAnimation {
                                    id: coreFillAnim
                                    target: centralCore; property: "disconnectFill"; to: 1.0
                                    duration: 700 * (1.0 - centralCore.disconnectFill); easing.type: Easing.InSine
                                    onFinished: {
                                        if (!coreMa.pressed) { centralCore.disconnectFill = 0.0; return; }
                                        centralCore.disconnectTriggered = true; centralCore.flashOpacity = 0.6;
                                        coreFlashAnim.start(); coreBumpAnim.start();
                                        audioManager.playSfx("network/disconnect.wav");
                                        ...
                                        centralCore.disconnectFill = 0.0; centralCore.disconnectTriggered = false;
                                        if (window.activeMode === "eth") ethPoller.running = true;
                                        else if (window.activeMode === "wifi") wifiPoller.running = true; 
                                        else btPoller.running = true;
                                    }
                                ...
                                NumberAnimation { id: coreDrainAnim; target: centralCore; property: "disconnectFill"; to: 0.0; duration: 1000 * centralCore.disconnectFill; easing.type: Easing.OutQuad }
                            }
            ...
            Item {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        property real singleLiveAngle: (window.globalOrbitAngle * 1.5) + singleBaseAngle + (Math.sin(window.microOrbitDrift + index) * 0.04)
                        ...
                        Rectangle {
                            ...
                            RowLayout {
                                ...
                                ColumnLayout {
                                    ...
                                    Text {
                                        ...
                                        text: floatCard.isFailed ? "Sync Failed" : (floatCard.isMyBusy ? "Syncing..." : (floatCard.renderFill > 0.1 && floatCard.renderFill < 1.0 ? floatCard.itemId === "ip_0" ? floatCard.triggered ? "Copied!" : "Hold to copy..." : "Hold..." : action))
                                        Behavior on color { ColorAnimation { duration: 200 } }
                                    }
                                }
                                ...
                                SatelliteLoader {
                                    ...
                                    visible: floatCard.isMyBusy || (floatCard.itemId === window.connectingId) || (window.disconnectingDevices[floatCard.itemId])
                                    accentColor: floatCard.isFailed ? window.red : window.activeColor
                                }
                            }
                            ...
                            Item {
                                ...
                                RowLayout {
                                    ...
                                    ColumnLayout {
                                        ...
                                        Item {
                                            Layout.fillWidth: true; height: window.s(18); clip: true
                                            
                                            Text {
                                                id: filledNameText
                                                anchors.left: parent.left; anchors.leftMargin: floatCard.textOffset; anchors.verticalCenter: parent.verticalCenter
                                                text: floatCard.itemName
                                                font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(13); color: window.crust 
                                            }
                                            Text { 
                                                anchors.left: filledNameText.right; anchors.leftMargin: window.s(30); anchors.verticalCenter: parent.verticalCenter
                                                visible: floatCard.doMarquee; text: floatCard.itemName
                                                font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(13); color: window.crust 
                                            }
                                        }
                                        Text {
                                            font.family: "JetBrains Mono"; font.pixelSize: window.s(10); color: window.crust
                                            text: floatCard.isMyBusy ? "Syncing..." : (floatCard.renderFill > 0.1 && floatCard.renderFill < 1.0 ? floatCard.itemId === "ip_0" ? floatCard.triggered ? "Copied!" : "Hold to copy..." : "Hold..." : action)
                                        }
                                    }
                                }
                            }
                            ...
                            NumberAnimation {
                                id: fillAnim
                                target: floatCard; property: "fillLevel"; to: 1.0
                                duration: 600 * (1.0 - floatCard.fillLevel); easing.type: Easing.InSine
                                onFinished: {
                                    floatCard.triggered = true; floatCard.flashOpacity = 0.6;
                                    cardFlashAnim.start(); cardBumpAnim.restart();
                            ...
                            NumberAnimation { id: drainAnim; target: floatCard; property: "fillLevel"; to: 0.0; duration: 1500 * floatCard.fillLevel; easing.type: Easing.OutQuad; onFinished: { if (isInfoNode && action === "IP Address") floatCard.triggered = false; } }
                        }
                    }
                }
            }
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        id: ethTabRect
                        Layout.fillWidth: true; Layout.fillHeight: true; visible: window.ethPresent; radius: window.s(10)
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                if (window.activeMode !== "eth") {
                                    window.powerAnimAllowed = false; powerAnimBlocker.restart();
                                    audioManager.playSfx("network/switch.wav"); window.activeMode = "eth";
                                }
                            }
                        }
                    }

                    Rectangle { visible: window.ethPresent && (window.wifiPresent || window.btPresent); width: 1; Layout.fillHeight: true; Layout.margins: window.s(5); color: "#22ffffff" }

                    Rectangle {
                        id: wifiTabRect
                        Layout.fillWidth: true; Layout.fillHeight: true; visible: window.wifiPresent; radius: window.s(10)
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                if (window.activeMode !== "wifi") {
                                    window.powerAnimAllowed = false; powerAnimBlocker.restart();
                                    audioManager.playSfx("network/switch.wav"); window.activeMode = "wifi";
                                }
                            }
                        }
                    }

                    Rectangle { visible: window.wifiPresent && window.btPresent; width: 1; Layout.fillHeight: true; Layout.margins: window.s(5); color: "#22ffffff" }

                    Rectangle {
                        id: btTabRect
                        Layout.fillWidth: true; Layout.fillHeight: true; visible: window.btPresent; radius: window.s(10)
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                if (window.activeMode !== "bt") {
                                    window.powerAnimAllowed = false; powerAnimBlocker.restart();
                                    audioManager.playSfx("network/switch.wav"); window.activeMode = "bt";
                                }
                            }
                        }
                    }
            ...
            Item {
                ...
                Item {
                    id: flatPowerToggleContainerChild
                    anchors.fill: parent
                    ...
                    Rectangle {
                        ...
                        Text {
                            ...
                            RotationAnimation {
                                target: pwrIcon; property: "rotation"; from: 0; to: 360; duration: 800; loops: Animation.Infinite
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                if (window.activeMode === "eth") {
                                    ...
                                    if (window.expectedEthPower === "on") audioManager.playSfx("network/power_on.wav"); else audioManager.playSfx("network/power_off.wav");
                                    ...
                                    if (window.expectedWifiPower === "on") audioManager.playSfx("network/power_on.wav"); else audioManager.playSfx("network/power_off.wav");
                                    ...
                                    if (window.expectedBtPower === "on") audioManager.playSfx("network/power_on.wav"); else audioManager.playSfx("network/power_off.wav");
